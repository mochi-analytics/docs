---
title: Ingest API
description: The HTTP wire contract for sending analytics from a bot to Mochi.
---
...
The wire contract for sending analytics from a bot to Mochi. The official SDKs
wrap this; any language can implement it directly.
...
<Callout type="warning">
  Limits: **100 events/request**, **256 KB body**. Events older than **48 h** or
  more than 5 minutes in the future are rejected per-event.
</Callout>
...
Errors: `400` validation (with zod issues), `401` bad/revoked key, `413`
oversized body, `429` rate limited (respect `Retry-After`).
...
`cpuPercent` (process CPU, normalized to 0–100 across all cores) and `memoryMb`
(resident set size in whole megabytes) are optional and power the CPU/memory
charts on the Health page. The SDKs measure both automatically on each snapshot;
pass your own values to override, or omit them to record only guild counts.

Snapshots are also what the [offline alert](/alerts#offline) watches: a bot that
stops sending them is a bot that looks down.

## Reading data back

The same API key authenticates the [Stats API](/stats-api), a read-only JSON
endpoint for pulling your bot's own metrics back out. Both share the rate limit
above.
